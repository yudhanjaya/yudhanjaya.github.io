...
nav_order: 3
date: 2024-02-01
image: "/assets/images/experiments/stonegarden/low-power-writing/low-power.png"
excerpt: "A minimalist writing station running at 24 watts: an Android gaming handheld, a wireless keyboard, and Obsidian. Built for distraction-free writing during Sri Lanka's power cuts."
---

# My Low Power Writing Setup
{: .no_toc }

*Updated April 2 2026*

{% include image.html image="/assets/images/experiments/stonegarden/low-power-writing/writing-setup.jpg" caption="Aha." %}

This is my writing setup.  I've got very decent PC there; a Ryzen5800 X3D with RTX 4080 Super, two four K displays, and quite a few custom keyboards. I have to keep a few Q tips handy so that I can clean the dust out of my keyboards. It's not much, but I do get paranoid about that stuff. Otherwise this is very comfortable.

However, this setup requires mains power being available. What if I wanted to create something that could run on as little energy as a light bulb? Off a power bank, for example.

{% include image.html image="/assets/images/experiments/stonegarden/low-power-writing/low-power.png" caption="My AYN Odin, hooked up to a keyboard and a monitor." %}

The image you see is my alternate writing setup.

It consumes very little power. Measured at the mains, it takes up 24 watts at full usage, including the monitor. Without the monitor, it runs as low as 4 watts, or at most at 10 watts. It has a 6,600 mAh battery, so it can run itself for a long time when the power goes out, and it has a 128 GB of onboard storage. It literally only requires as much power as a light bulb. Even less, if we're talking CFL.

I built this for multiple intersecting reasons:

- I wanted to write without distractions.
- I wanted a low-power writing setup that could run on a battery for a long time
...
- I wanted it to be cheap to run on solar
...
- A $40 Ugreen dock/adapter, a charger, and some USB-C and HDMI cables.
...
Obsidian is a note-taking, second-brain sort of software that can be customized to a near-infinite degree. My Obsidian setup exists across all my machines. I use it to take notes, write my books, and everything in between. Even this note is written in Obsidian. It writes to Markdown, which is essentially just a text file. You can check it out at [Obsidian.md](https://obsidian.md).
